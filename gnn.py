...
if __name__ == "__main__":
    ...
    #     agent_0 = IMAgent(0, "random", 5, transmission_probability=0.1, color="red")
    #     # agent_1 = IMAgent(1, "random", 5, transmission_probability=0.1, color="blue")
    #     # agent_2 = IMAgent(2, "random", 5, transmission_probability=0.1, color="green")
    #     # agent_3 = IMAgent(3, "random", 5, transmission_probability=0.1, color="yellow")
    #     agents = {0: agent_0}
    #         #   1: agent_1,
    #         #   2: agent_2,
    #         #   3: agent_3}
        
    #     for num_agent in range(1,5):
    #         agents[num_agent] = IMAgent(num_agent, "random", 5, transmission_probability=0.1, color=colors[num_agent])
    #         best_accs = np.array([])
    #         for _ in range(num_experiments):
    #             best_acc = mini_graphs_experiment(method, agents, False)
    #             best_accs = np.append(best_accs, best_acc)
    #         mean = best_accs.mean()
    #         std = best_accs.std()
    #         standard_error = std / np.sqrt(num_experiments)
    #         margin_error = standard_error * 1.96 # 95% interval
    #         print(method, len(agents), round(mean,3), round(margin_error,3))

        # for num_agent in range(1,5):
        #     agents[num_agent] = IMAgent(num_agent, "random", 6-num_agent, transmission_probability=0.1, color=colors[num_agent])
        #     for m in range(2,5):
        #         best_accs = np.array([])
        #         for _ in range(num_experiments):
        #             agent_copy = get_agents_copy(agents)
        #             best_acc = multi_ba_expirement(50, m, method, agent_copy, False)
        #             best_accs = np.append(best_accs, best_acc)
        #         mean = best_accs.mean()
        #         std = best_accs.std()
        #         standard_error = std / np.sqrt(num_experiments)
        #         margin_error = standard_error * 1.96 # 95% interval
        #         print(method, len(agents), m, round(mean,3), round(margin_error,3))

    num_nodes_selected = 5
    agent_0 = IMAgent(0, "random", num_nodes_selected, transmission_probability=0.1, color="red")
    agent_1 = IMAgent(1, "random", num_nodes_selected, transmission_probability=0.1, color="blue")
    agent_2 = IMAgent(2, "random", num_nodes_selected, transmission_probability=0.1, color="green")
    agent_3 = IMAgent(3, "random", num_nodes_selected, transmission_probability=0.1, color="yellow")
    agents = {0: agent_0,
              1: agent_1}
            #   2: agent_2,
            #   3: agent_3}
    mini_graphs_experiment("deterministic", agents, True)
    # multi_ba_expirement(50, 3, "deterministic", agents, True)
